...
arma::vec v_surp( arma::vec d, arma::vec coeff, arma::vec G, bool tri ){
  ...
  vec out = zeros(n) ;
  for ( int i=0 ; i < n ; i++ ){
    out[i] = surp( d[i], coeff, G[i], tri ) ;
  }
  return out ;
}

// [[Rcpp::export]]
double surp( double d, arma::vec coeff, double G=0, bool tri=false ){
  return tri ? surp_tri( d, coeff, G ) : surp_poly( d, coeff, G ) ;
}

// [[Rcpp::export]]
double surp_poly( double d, arma::vec coeff, double G=0 ){
...
// [[Rcpp::export]]
double surp_tri( double d, arma::vec coeff, double G=0 ){
...
// [[Rcpp::export]]
double d_surp_tri( double d, arma::vec coeff, double G=0 ){
  double x1 = coeff(0) ;
  double x2 = coeff(1) ;
  double x3 = coeff(2) ;
  double y1 = coeff(3) ;
